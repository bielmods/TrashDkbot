	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola seja bem vindo @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '❌ Este comando só pode ser usado em grupos mano! ❌',
					ownerG: '❌ Este comando só pode ser usado pelos adms do grupo ❌',
					ownerB: '❌ Este comando só pode ser usado pelo dono do bot seu burro >:( ❌',
					admin: '❌ Este comando só pode ser usado por adms do grupo😔🤙 ❌',
					Badmin: '❌ Este comando só pode ser usado quando você é adm e o bot também 😔🤙 ❌'
				}
			}
			...
			switch(command) {
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde esta o nome de usuário?', text, {quoted: mek})
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você quer expulsar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ apenas sticker em imagem ❌')
					reply(mess.wait)
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Onde está o texto poha, hum?')
					teks = body.slice(5)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo simih ativado com sucesso ✔️')
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo boas vindas ativado com sucesso neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Modo boas vindas desativado com sucesso ✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
                                      break
